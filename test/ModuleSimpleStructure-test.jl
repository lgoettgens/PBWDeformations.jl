@testset "ModuleSimpleStructure.jl tests" begin
    @testset for L in [general_linear_lie_algebra(QQ, 3), special_orthogonal_lie_algebra(QQ, 3, identity_matrix(QQ, 3))]
        ...
        @testset "Duality, exterior power, k = $k" for k in 2:3
            V = dual(exterior_power_obj(stdV, k))
            W, h = isomorphic_module_with_simple_structure(V)
            @test is_welldefined(h)
            @test is_isomorphism(h)
        ...
        @testset "Duality, symmetric power, k = $k" for k in 2:5
            V = dual(symmetric_power_obj(stdV, k))
            W, h = isomorphic_module_with_simple_structure(V)
            @test is_welldefined(h)
            @test is_isomorphism(h)
        ...
        @testset "Duality, tensor power, k = $k" for k in 2:5
            V = dual(tensor_power_obj(stdV, k))
            W, h = isomorphic_module_with_simple_structure(V)
            @test is_welldefined(h)
            @test is_isomorphism(h)
        ...
        @testset "Direct sum: one summand" begin
            V = direct_sum(exterior_power_obj(stdV, 2))
            W, h = isomorphic_module_with_simple_structure(V)
            @test is_welldefined(h)
            @test is_isomorphism(h)
        ...
        @testset "Direct sum: 0-dim summand" begin
            V1 = dual(stdV)
            V3 = exterior_power_obj(stdV, 2)
            for V2 in [exterior_power_obj(stdV, dim(stdV) + 1)]
        ...
        @testset "Direct sum: nested" begin
            V = direct_sum(direct_sum(stdV, exterior_power_obj(stdV, 2)), dual(stdV))
            W, h = isomorphic_module_with_simple_structure(V)
            @test is_welldefined(h)
            @test is_isomorphism(h)
        ...
        @testset "Tensor product: one factor" begin
            V = tensor_product(exterior_power_obj(stdV, 2))
            W, h = isomorphic_module_with_simple_structure(V)
            @test is_welldefined(h)
            @test is_isomorphism(h)
            @test W == exterior_power_obj(stdV, 2)
        ...
        @testset "Tensor product: trivial 1-dim factor" begin
            V1 = dual(stdV)
            V3 = exterior_power_obj(stdV, 2)
            for V2 in [tensor_power_obj(stdV, 0), exterior_power_obj(stdV, 0), symmetric_power_obj(stdV, 0)]
        ...
        @testset "Tensor product: nested" begin
            V = tensor_product(tensor_product(stdV, exterior_power_obj(stdV, 2)), dual(stdV))
            W, h = isomorphic_module_with_simple_structure(V)
            @test is_welldefined(h)
            @test is_isomorphism(h)
        ...
        @testset "Tensor product, direct sum" begin
            V = tensor_product(direct_sum(stdV, symmetric_power_obj(stdV, 2)), direct_sum(dual(stdV), stdV))
            W, h = isomorphic_module_with_simple_structure(V)
            @test is_welldefined(h)
            @test is_isomorphism(h)
            @test W == direct_sum(
                ...
                tensor_product(symmetric_power_obj(stdV, 2), dual(stdV)),
                tensor_product(symmetric_power_obj(stdV, 2), stdV),
            )

            V1 = exterior_power_obj(stdV, 2)
            V2 = dual(stdV)
            V3 = symmetric_power_obj(stdV, 2)
            V4 = dual(stdV)
            V5 = stdV
            V6 = symmetric_power_obj(stdV, 2)
        ...
        @testset "Exterior power, direct sum" begin
            ...
            V2 = dual(stdV)
            V3 = exterior_power_obj(stdV, 2)
            V = exterior_power_obj(direct_sum(V1, V2, V3), 3)
            W, h = isomorphic_module_with_simple_structure(V)
            @test is_welldefined(h)
            @test is_isomorphism(h)
            @test W == direct_sum(
                exterior_power_obj(V1, 3),
                tensor_product(exterior_power_obj(V1, 2), V2),
                tensor_product(exterior_power_obj(V1, 2), V3),
                tensor_product(V1, exterior_power_obj(V2, 2)),
                tensor_product(V1, V2, V3),
                tensor_product(V1, exterior_power_obj(V3, 2)),
                exterior_power_obj(V2, 3),
                tensor_product(exterior_power_obj(V2, 2), V3),
                tensor_product(V2, exterior_power_obj(V3, 2)),
                exterior_power_obj(V3, 3),
            )
        end

        @testset "Symmetric power: 1st power" begin
            V1 = stdV
            V = symmetric_power_obj(V1, 1)
            W, h = isomorphic_module_with_simple_structure(V)
            @test is_welldefined(h)
            @test is_isomorphism(h)
            @test W == V1
        ...
        @testset "Symmetric power, direct sum" begin
            ...
            V2 = dual(stdV)
            V3 = exterior_power_obj(stdV, 2)
            V = symmetric_power_obj(direct_sum(V1, V2, V3), 3)
            W, h = isomorphic_module_with_simple_structure(V)
            @test is_welldefined(h)
            @test is_isomorphism(h)
            @test W == direct_sum(
                symmetric_power_obj(V1, 3),
                tensor_product(symmetric_power_obj(V1, 2), V2),
                tensor_product(symmetric_power_obj(V1, 2), V3),
                tensor_product(V1, symmetric_power_obj(V2, 2)),
                tensor_product(V1, V2, V3),
                tensor_product(V1, symmetric_power_obj(V3, 2)),
                symmetric_power_obj(V2, 3),
                tensor_product(symmetric_power_obj(V2, 2), V3),
                tensor_product(V2, symmetric_power_obj(V3, 2)),
                symmetric_power_obj(V3, 3),
            )
        end

        @testset "Tensor power: 1st power" begin
            V1 = stdV
            V = tensor_power_obj(V1, 1)
            W, h = isomorphic_module_with_simple_structure(V)
            @test is_welldefined(h)
            @test is_isomorphism(h)
            @test W == V1
        ...
        @testset "Tensor power, direct sum" begin
            V1 = stdV
            V2 = exterior_power_obj(dual(stdV), 2)
            V3 = exterior_power_obj(stdV, 2)
            V = tensor_power_obj(direct_sum(V1, V2, V3), 3)
            W, h = isomorphic_module_with_simple_structure(V)
            # @test is_welldefined(h)   # needs 3 minutes
            @test is_isomorphism(h)
            @test W == direct_sum(
                tensor_power_obj(V1, 3),                        # 111
                tensor_product(tensor_power_obj(V1, 2), V2),    # 112
                tensor_product(tensor_power_obj(V1, 2), V3),    # 113
                tensor_product(tensor_power_obj(V1, 2), V2),    # 121
                tensor_product(V1, tensor_power_obj(V2, 2)),    # 122
                tensor_product(V1, V2, V3),                 # 123
                tensor_product(tensor_power_obj(V1, 2), V3),    # 131
                tensor_product(V1, V2, V3),                 # 132
                tensor_product(V1, tensor_power_obj(V3, 2)),    # 133
                tensor_product(tensor_power_obj(V1, 2), V2),    # 211
                tensor_product(V1, tensor_power_obj(V2, 2)),    # 212
                tensor_product(V1, V2, V3),                 # 213
                tensor_product(V1, tensor_power_obj(V2, 2)),    # 221
                tensor_power_obj(V2, 3),                        # 222
                tensor_product(tensor_power_obj(V2, 2), V3),    # 223
                tensor_product(V1, V2, V3),                 # 231
                tensor_product(tensor_power_obj(V2, 2), V3),    # 232
                tensor_product(V2, tensor_power_obj(V3, 2)),    # 233
                tensor_product(tensor_power_obj(V1, 2), V3),    # 311
                tensor_product(V1, V2, V3),                 # 312
                tensor_product(V1, tensor_power_obj(V3, 2)),    # 313
                tensor_product(V1, V2, V3),                 # 321
                tensor_product(tensor_power_obj(V2, 2), V3),    # 322
                tensor_product(V2, tensor_power_obj(V3, 2)),    # 323
                tensor_product(V1, tensor_power_obj(V3, 2)),    # 331
                tensor_product(V2, tensor_power_obj(V3, 2)),    # 332
                tensor_power_obj(V3, 3),                        # 333
            )
        end
        ...
        @testset "Complex cases 1" begin
            ...
            V_1 = exterior_power_obj(V3, 1)
            W_1, h = isomorphic_module_with_simple_structure(V_1)
            @test is_welldefined(h)
            @test is_isomorphism(h)
            @test W_1 == V3

            V_2 = exterior_power_obj(V3, 2)
            W_2, h = isomorphic_module_with_simple_structure(V_2)
            @test is_welldefined(h)
            @test is_isomorphism(h)
            @test W_2 == direct_sum(exterior_power_obj(V1, 2), tensor_product(V1, V2), exterior_power_obj(V2, 2))

            V_3 = exterior_power_obj(V3, 3)
            ...
            W_4b, hb = isomorphic_module_with_simple_structure(
                direct_sum(
                    exterior_power_obj(V1, 4),
                    tensor_product(exterior_power_obj(V1, 3), V2),
                    tensor_product(exterior_power_obj(V1, 2), exterior_power_obj(V2, 2)),
                    tensor_product(V1, exterior_power_obj(V2, 3)),
                    exterior_power_obj(V2, 4),
                ),
            )
        ...
        @testset "Complex cases 2" begin
            V1 = symmetric_power_obj(stdV, 3)

            Ve2 = dual(exterior_power_obj(dual(V1), 2))
            We2, h = isomorphic_module_with_simple_structure(Ve2)
            @test is_welldefined(h)
            @test is_isomorphism(h)
            @test We2 == exterior_power_obj(V1, 2)

            Ve3 = dual(exterior_power_obj(dual(V1), 3))
            We3, h = isomorphic_module_with_simple_structure(Ve3)
            @test is_welldefined(h)
            @test is_isomorphism(h)
            @test We3 == exterior_power_obj(V1, 3)

            Vs2 = dual(symmetric_power_obj(dual(V1), 2))
            Ws2, h = isomorphic_module_with_simple_structure(Vs2)
            @test is_welldefined(h)
            @test is_isomorphism(h)
            @test Ws2 == symmetric_power_obj(V1, 2)

            Vt2 = dual(tensor_power_obj(dual(V1), 2))
            Wt2, h = isomorphic_module_with_simple_structure(Vt2)
            @test is_welldefined(h)
            @test is_isomorphism(h)
            @test Wt2 == tensor_power_obj(V1, 2)
        end
    end
end
