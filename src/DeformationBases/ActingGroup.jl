function acting_group_with_sgn(T::Union{SO, GL}, V::LieAlgebraModule)
    G, gens_and_sgn = _acting_group_with_gens_and_sgn(T, V)
...
function _acting_group_with_gens_and_sgn(T::Union{SO, GL}, V::LieAlgebraModule)
    ...
    elseif ((fl, inner_mods) = _is_tensor_product(V); fl)
        inner_stuff = [_acting_group_with_gens_and_sgn(T, inner_mod) for inner_mod in inner_mods]
    ...
    elseif ((fl_ext, inner_mod, power) = _is_exterior_power(V); fl_ext) || ((fl_sym, inner_mod, power) = _is_symmetric_power(V); fl_sym)
        inner_group, inner_gens_and_sgn = _acting_group_with_gens_and_sgn(T, inner_mod)
        if power == 1
            return inner_group, inner_gens_and_sgn
        end
        top_group = symmetric_group(power)
        W = wreath_product(inner_group, top_group) # maybe replace by GAP.Globals.WreathProductImprimitiveAction
