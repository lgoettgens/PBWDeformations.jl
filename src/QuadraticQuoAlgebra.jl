mutable struct QuadraticQuoAlgebra{C <: RingElement} <: Algebra{C}
    ...
    function QuadraticQuoAlgebra{C}(
        free_alg::FreeAlgebra{C},
...
mutable struct QuadraticQuoAlgebraElem{C <: RingElement} <: AlgebraElem{C}
    ...
    function QuadraticQuoAlgebraElem{C}(
        A::QuadraticQuoAlgebra,
        c::Vector{C},
        m::Vector{Vector{Int}},
    ) where {C <: RingElement}
    ...
    function QuadraticQuoAlgebraElem{C}(
        A::QuadraticQuoAlgebra{C},
        b::QuadraticQuoAlgebraElem{C},
    ) where {C <: RingElement}
        A.free_alg === parent(b).free_alg || throw(ArgumentError("Non-matching algebras"))
        return new{C}(deepcopy(b.coeffs), deepcopy(b.monoms), length(b.coeffs), A)
    end

    function QuadraticQuoAlgebraElem{C1}(
        A::QuadraticQuoAlgebra{C1},
        b::QuadraticQuoAlgebraElem{C2},
...
Base.:(==)(a::QuadraticQuoAlgebraElem{C}, b::QuadraticQuoAlgebraElem{C}; strict::Bool=false) where {C <: RingElement} =
    isequal(a, b; strict)

function Base.isequal(
    a::QuadraticQuoAlgebraElem{C},
    b::QuadraticQuoAlgebraElem{C};
    strict::Bool=false,
) where {C <: RingElement}
...
Base.:(==)(n::Union{Integer, Rational, AbstractFloat}, a::QuadraticQuoAlgebraElem; strict::Bool=false) =
    isequal(a, n; strict)
Base.isequal(n::Union{Integer, Rational, AbstractFloat}, a::QuadraticQuoAlgebraElem; strict::Bool=false) =
    isequal(a, n; strict)
Base.:(==)(a::QuadraticQuoAlgebraElem, n::Union{Integer, Rational, AbstractFloat}; strict::Bool=false) =
    isequal(a, n; strict)
